...
fn bench_linear_backward(c: &mut Criterion) {
    ...
    group.bench_function(
        BenchmarkId::new("bwd", format!("{batch}x{d_in}->{d_out}")),
        |b| {
...
fn bench_gqa(c: &mut Criterion) {
    ...
    for &(seq, d_model, n_heads, n_kv) in &[(8, 64, 4, 2), (32, 64, 4, 2)] {
        let mut gqa =
            GroupedQueryAttention::<f64>::new(d_model, n_heads, n_kv, 42).with_causal(true);
...
fn bench_rope(c: &mut Criterion) {
    ...
    for &(seq, dim) in &[(32, 64), (128, 64)] {
        ...
        group.bench_function(BenchmarkId::new("apply", format!("seq{seq}_d{dim}")), |b| {
...
fn bench_cross_entropy(c: &mut Criterion) {
    ...
    for &(seq, vocab) in &[(32, 256), (128, 256)] {
        ...
        let targets = Tensor::from_fn(Shape::from_slice(&[seq]), |idx| (idx[0] % vocab) as f64);
        group.bench_function(
...
fn bench_kv_cache(c: &mut Criterion) {
    ...
    group.bench_function("prefill_128tok", |b| {
        b.iter(|| {
            let mut cache =
                tang_infer::KVCache::<f64>::new(num_layers, max_seq, n_kv_heads, head_dim);
    ...
    group.bench_function("decode_128steps", |b| {
        b.iter(|| {
            ...
            for step in 0..128 {
                let k = Tensor::new(
                    vec![step as f64 * 0.01; kv_dim],
                    Shape::from_slice(&[1, kv_dim]),
                );
                let v = Tensor::new(
                    vec![step as f64 * 0.02; kv_dim],
                    Shape::from_slice(&[1, kv_dim]),
                );
                for layer in 0..num_layers {
                    cache.append(layer, &k, &v);
...
fn bench_sampling(c: &mut Criterion) {
    ...
    group.bench_function("topk40_v256", |b| {
        let mut sampler =
            tang_infer::Sampler::with_seed(tang_infer::SamplingConfig::standard(), 42);
        b.iter(|| black_box(sampler.sample(&logits, &[])));
    });
